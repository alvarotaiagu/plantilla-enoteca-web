...
(function () {
  ...
  (function cortina() {
    ...
    var vino = el.querySelector('.cortina__vino');
    var centro = el.querySelector('.cortina__centro');
    gsap.set(centro, { opacity: 0, y: 10 });
    var tl = gsap.timeline({ onComplete: quitar });
    tl.to(centro, { opacity: 1, y: 0, duration: 0.42, ease: 'power2.out' })
      .fromTo(vino, { scaleY: 0 },
        { scaleY: 1, duration: 0.62, ease: 'power2.inOut', transformOrigin: '50% 100%', immediateRender: false }, '-=0.18')
      .to(centro, { opacity: 0, duration: 0.28, ease: 'power1.in' }, '+=0.1')
      .to(el, { yPercent: -104, duration: 0.95, ease: 'expo.inOut' }, '-=0.14');
